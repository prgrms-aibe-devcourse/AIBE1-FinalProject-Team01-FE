...
const AVAILABLE_TOPICS = Object.entries(TOPIC_MAPPING).map(([label, key]) => ({
  key,
  label,
}));
...
export const EditProfileForm = ({ onSave, onCancel, initial }) => {
  ...
  const loadCurrentProfile = async () => {
    try {
      ...
      const safeTopics = Array.isArray(initial.topics) ? initial.topics : [];
  ...
  const checkNicknameDuplicate = async () => {
    ...
    try {
      ...
      const response = await accountApi.checkNicknameDuplicate(
        formData.nickname
      );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (
        JSON.stringify(formData.topics.sort()) !==
        JSON.stringify(originalData.topics.sort())
      ) {
            ...
            <div style={{ flex: 1, position: "relative" }}>
              <Form.Control
                ...
                style={{
                  fontSize: "1rem",
                  paddingRight:
                    nicknameStatus === "available" ? "40px" : "12px",
                }}
                required
              />
              {/* 입력 필드 내 체크 아이콘 */}
